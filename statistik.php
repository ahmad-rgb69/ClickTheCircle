...
<div class="w-full max-w-[1100px] mx-auto">
    <div class="flex items-center justify-between gap-3 mb-4">
        <h2 class="h-section m-0">📊 Statistik Permainan</h2>
        <a href="lobby.php" class="btn">← Kembali ke Lobby</a>
    </div>
    ...
    <?php if ($sessionId > 0):
        ...
        // ---- Daftar riwayat (semua sesi yg dimiliki user) ----
        $stmt = mysqli_prepare($db, "SELECT gs.id, gs.room_id, gs.difficulty, gs.num_players, gs.duration_sec, gs.started_at, gs.ended_at,
                                        (SELECT player_label FROM game_scores WHERE session_id = gs.id AND is_winner=1 ORDER BY score DESC LIMIT 1) AS winner_label,
        ...
        <div class="panel-box p-4">
            <p class="text-sm mb-3">Menampilkan riwayat sesi sebagai <strong>owner</strong> (kamu yang membuat ruangan saat
                sesi berjalan).</p>
            <?php if (empty($rows)): ?>
