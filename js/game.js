...
(() => {
    ...
    const DIFF = {
        easy: {
            ...
            turboRegen: 30,
            playerSteal: false,
            // Lebih banyak normal/bonus, jarang trap.
            boxOverrides: {
                normal: {
                    weight: 0.65
                },
        ...
        indonesian: {
            // Mode paling susah: arena kecil, semua serba cepat, hampir tidak ada
            // bonus, traps mendominasi, dan tabrakan antar pemain bisa mencuri poin.
            arenaW: 600,
            arenaH: 360,
            boxSize: BOX_SIZE - 4,
            baseSpeed: 3.4,
    ...
    function effectiveBoxTypes(diffKey) {
        ...
        return BOX_TYPES.map(t => ov[t.kind] ? {
            ...t,
            ... ov[t.kind]
        } : t);
    }
    ...
    function ensureNormalInvariant(cfg) {
        if (! state.boxes.some(b => b.kind === 'normal')) { // ganti box paling tua dengan normal supaya jumlah tetap.
            let oldestIdx = 0,
                oldestAge = -1;
            ...
            if (state.boxes.length === 0) 
                state.boxes.push(spawnBox(cfg, {kind: 'normal'}));
             else { // reset target AI yang mengarah ke slot ini
                for (const other of state.players) {
                    if (other.ai && other.ai.targetIdx === oldestIdx) {
                        other.ai.targetIdx = -1;
                        other.ai.retargetAt = 0;
                    }
                }
    ...
    function expireOldBoxes(cfg) {
        ...
        for (let i = 0; i < state.boxes.length; i++) {
            ...
            if (age >= BOX_LIFETIME_MS) {
                for (const other of state.players) {
                    if (other.ai && other.ai.targetIdx === i) {
                        other.ai.targetIdx = -1;
                        other.ai.retargetAt = 0;
                    }
                }
    ...
    // render 100 ms di belakang snapshot terbaru -> mulus
    ...
    const renderState = {
        // hasil interpolasi yang DIGAMBAR
        players: [],
        boxes: [],
        timeLeft: 0,
        arenaW: 900,
    ...
    function pushSnapshot(snap) {
        ...
        while (snapBuf.length > 2 && snapBuf[0].t < cutoff) 
            snapBuf.shift();
    ...
    function sampleRenderState() {
        ...
        if (canvas.width !== renderState.arenaW || canvas.height !== renderState.arenaH) {
            canvas.width = renderState.arenaW;
            canvas.height = renderState.arenaH;
        }
    }
    ...
    function mySeatNo() { // Cari seat saya dari __SEAT_STATE__
        const seats = (window.__SEAT_STATE__ && window.__SEAT_STATE__.seats) || [];
    ...
    function predictTick(dt) {
        ...
        if (localPred.x < localPred.r) 
            localPred.x = localPred.r;
        


        if (localPred.y < localPred.r) 
            localPred.y = localPred.r;
        


        if (localPred.x > canvas.width - localPred.r) 
            localPred.x = canvas.width - localPred.r;
        


        if (localPred.y > canvas.height - localPred.r) 
            localPred.y = canvas.height - localPred.r;
    ...
    function pushLocalSnapshotForOwner() {
        pushSnapshot({
            ...
            boxes: state.boxes.map(b => ({
                ...
                kind: b.kind,
                color: b.color,
                age: Math.max(0, performance.now() - (b.bornAt || 0))
            }))
        });
    }
    ...
    function inputForPlayer(p) {
        ...
        return remoteInputs.get(p.userId) || {
            up: false,
            down: false,
            left: false,
            right: false,
    ...
    function botBoxValue(p, box, distSq) {
        ...
        if (per === 'cautious') {
            if (t.score < 0 || t.kind === 'freeze' || t.kind === 'slow') 
                v -= 80;
    ...
    function botInputFor(p) {
        ...
        if (now >= ai.retargetAt) {
            ...
            for (let i = 0; i < state.boxes.length; i++) {
                ...
                if (claimed.has(i) && ai.personality !== 'greedy') 
                    val -= 40;
        ...
        if (ai.personality === 'greedy') 
            useTurbo = distSqToTarget > 140 * 140 && p.turbo > 25;
         else if (ai.personality === 'cautious') 
            useTurbo = distSqToTarget > 260 * 260 && p.turbo > 60;
         else if (ai.personality === 'opportunist') 
            useTurbo = distSqToTarget > 200 * 200 && p.turbo > 40;
         else 
            useTurbo = Math.random() < 0.05 && p.turbo > 30;
    ...
    function ownerUpdate(dt) {
        const cfg = DIFF[state.diff];
        const now = performance.now();
        for (const p of state.players) { // Frozen: tidak bisa bergerak sama sekali
            ...
            if (autoBoost) {
                ...
                if (p.turbo > cfg.turboMax) 
                    p.turbo = cfg.turboMax;
        ...
        if (cfg.playerSteal) {
            ...
            for (let i = 0; i < players.length; i++) {
                ...
                for (let j = i + 1; j < players.length; j++) {
                    const b = players[j];
                    if (nowMs < (a.frozenUntil || 0) || nowMs < (b.frozenUntil || 0)) 
                        continue;
                    ...
                    let attacker = null,
                        victim = null;
                    if (a.turboActive && ! b.turboActive) {
                        attacker = a;
                        victim = b;
                    } else if (b.turboActive && ! a.turboActive) {
                        attacker = b;
                        victim = a;
                    } else 
                        continue;
                     attacker.stealCdUntil = attacker.stealCdUntil || {};
                    if (nowMs < (attacker.stealCdUntil[victim.seat] || 0)) 
                        continue;
        ...
        for (let i = 0; i < state.boxes.length; i++) {
            ...
            for (const p of state.players) {
                ...
                if (ddx * ddx + ddy * ddy <= p.r * p.r) {
                    ...
                    const nowMs = performance.now();
                    if (t.freezeMs) 
                        p.frozenUntil = nowMs + t.freezeMs;
                    


                    if (t.slowMs) {
                        p.slowUntil = nowMs + t.slowMs;
                        p.slowMul = t.slowMul || 0.55;
                    }
                    if (t.turboFill) {
                        p.turbo = Math.min(cfg.turboMax, (p.turbo | 0) + t.turboFill);
                    }
                    if (t.boostMs) 
    ...
    function broadcastSnapshot() {
        sendWS({
            ...
            boxes: state.boxes.map(b => ({
                ...
                kind: b.kind,
                color: b.color,
                age: Math.round(Math.max(0, performance.now() - (b.bornAt || 0)))
            }))
        });
    }
    ...
    function ownerEnd() {
        ...
        const msg = tie.length > 1 ? 'Seri: ' + tie.map(p => p.name).join(', ') : 'Pemenang: ' + top.name + ' (' + top.score + ' poin)';
    ...
    function applyRemoteSnapshot(d) { // Non-owner: simpan ke buffer; renderLoop akan interpolasi.
    ...
    function drawFromRenderState() {
        ...
        if (localPred.active) {
            drawPlayers = players.map(p => p.seat === localPred.seat ? {
                ...p,
                x: localPred.x,
                y: localPred.y
            } : p);
        ...
        for (const p of drawPlayers) {
            ...
            if (av && av.complete && av.naturalWidth > 0) {
                ...
                ctx.restore();
            } else { // Fallback: lingkaran berwarna + huruf inisial
                ctx.beginPath();
                ctx.arc(p.x, p.y, r, 0, Math.PI * 2);
    ...
    function renderScoresFromRender() {
        ...
        for (const p of list) {
            ...
            row.innerHTML = `
        <span class="inline-block w-3 h-3 rounded-full border border-ink" style="background:${
                p.color
            }"></span>
        <span class="font-bold">${
                escapeHtml(p.name)
            }</span>
        <span class="ml-1">Skor: <strong>${
                p.score | 0
            }</strong></span>
    ...
    function updateTimerUI() {
        ui.timer.textContent = (state.timeLeft | 0) + 's';
    }
    ...
    // ---- Lock layar ke arena saat game berjalan (semua device, termasuk Android) ----
    let _lockedScrollY = 0;
    function lockScreenToArena() {
        ...
        if (b.classList.contains('game-screen-locked')) 
            return;
        


        // 1) Scroll ke paling bawah halaman supaya arena ada di atas viewport
        //    dan joystick / tombol turbo (fixed bottom) tidak menutupi area arena.
        const maxY = Math.max(0, (document.documentElement.scrollHeight || document.body.scrollHeight || 0) - window.innerHeight);
        const targetY = maxY;

        // 2) Scroll instan ke target lalu langsung kunci pada posisi tersebut.
        try {
            window.scrollTo({top: targetY, left: 0, behavior: 'instant'});
        } catch (_) {
            window.scrollTo(0, targetY);
        }
        _lockedScrollY = targetY;

        b.dataset._lockY = String(_lockedScrollY);
        b.classList.add('game-screen-locked');
        b.style.position = 'fixed';
        b.style.top = `-${_lockedScrollY}px`;
        b.style.left = '0';
        b.style.right = '0';
        b.style.width = '100%';
        document.documentElement.classList.add('game-screen-locked');
        b.style.overflow = 'hidden';
        document.documentElement.style.overflow = 'hidden';
        // Cegah pinch / pull-to-refresh di Android
    ...
    function unlockScreen() {
        ...
        b.classList.remove('game-screen-locked');
        b.style.position = '';
        b.style.top = '';
        b.style.left = '';
        b.style.right = '';
        ...
        document.documentElement.classList.remove('game-screen-locked');
        document.removeEventListener('touchmove', _preventTouchMove, {passive: false});
        const y = parseInt(b.dataset._lockY || '0', 10) || 0;
        delete b.dataset._lockY;
        window.scrollTo(0, y);
    }
    function _preventTouchMove(e) { // Izinkan touch di area arena & kontrol mobile (joystick, tombol turbo, modal akhir).
        const t = e.target;
        ...
        if (t.closest('#game-canvas, #mobile-joystick, #floating-controls, #end-modal, #chat-drawer')) 
            return;
    ...
    function setRoomControlsLocked(locked) {
        ...
        window.dispatchEvent(new CustomEvent('room-controls-lock', {
            detail: {
                locked: !! locked
            }
        }));
    ...
    // Bersihkan arena: hapus snapshot buffer, kosongkan renderState & state,
    // lalu cat canvas hitam polos. Dipakai oleh Reset & saat menutup modal end.
    function clearArenaToBlack() {
        state.running = false;
        if (typeof stopInputSender === 'function') 
            stopInputSender();
        ...
        state.players = [];
        state.boxes = [];
        if (typeof remoteInputs !== 'undefined' && remoteInputs.clear) 
            remoteInputs.clear();
        ...
        renderState.players = [];
        renderState.boxes = [];
        renderState.timeLeft = 0;
        if (typeof localPred !== 'undefined') 
            localPred.active = false;
        ...
        if (ui.status) 
            ui.status.textContent = 'Arena dikosongkan. Tekan Mulai untuk main lagi.';
        


    }

    if (endOkBtn) { // OK = close modal, lepas lock layar, dan bersihkan arena (owner & non-owner).
        endOkBtn.addEventListener('click', () => {
            closeEndModal();
            unlockScreen();
            clearArenaToBlack();
            if (isOwner) { // Pastikan non-owner ikut bersih + flag "sedang main" di server di-clear.
                sendWS({type: 'game_reset', room_id: RC.roomId});
            }
        });
    }

    function triggerOwnerReset() { // Owner reset: kosongkan arena jadi hitam + broadcast supaya non-owner ikut bersih.
        clearArenaToBlack();
        state.timeLeft = gameDuration;
        updateTimerUI();
        setRoomControlsLocked(false);
        closeEndModal();
        // Kirim snapshot kosong supaya non-owner buffer tidak pakai data lama, lalu sinyal reset.
        sendWS({
            type: 'game_state',
            room_id: RC.roomId,
            diff: state.diff,
    ...
    window.addEventListener('ws-game-msg', (ev) => {
        ...
        if (isOwner) { // Owner menerima input dari remote players
        ...
        } else { // Non-owner: terima snapshot, started, ended
            if (d.type === 'game_state') 
                applyRemoteSnapshot(d);
                ...
                if (d.diff) {
                    state.diff = d.diff;
                    ui.diffSel.value = d.diff;
                }
                if (d.duration) {
                    gameDuration = d.duration;
                    if (ui.durSel) 
                        ui.durSel.value = String(d.duration);
                    


                    state.timeLeft = d.duration;
                    updateTimerUI();
                }
                closeEndModal();
                // jika modal masih terbuka dari ronde sebelumnya, tutup
                // Auto-scroll ke arena & lock layar untuk SEMUA pemain (termasuk Android)
                setRoomControlsLocked(true);
                startBGM();
            } else if (d.type === 'sfx_box') {
                playBoxSfx();
            ...
            } else if (d.type === 'game_diff') {
                if (d.diff) {
                    ui.diffSel.value = d.diff;
                    state.diff = d.diff;
                }
            } else if (d.type === 'game_duration') {
                if (d.duration) {
                    gameDuration = d.duration;
                    if (ui.durSel) 
                        ui.durSel.value = String(d.duration);
    ...
    if (isOwner) {
        ...
        ui.resetBtn.addEventListener('click', () => { // Owner reset: kosongkan arena jadi hitam (semua pemain) + clear flag "sedang main".
            clearArenaToBlack();
            ...
            // Snapshot kosong supaya non-owner tidak menggambar dari buffer lama.
            sendWS({
                type: 'game_state',
                room_id: RC.roomId,
                diff: state.diff,
    ...
    } else { // Non-owner mulai kirim input begitu di-load
        startInputSender();
    }
    ...
    canvas.width = cfg0.arenaW;
    canvas.height = cfg0.arenaH;
    setRoomControlsLocked(false);
    drawAll();
    updateTimerUI();
    ...
    function sfxEnabled() {
        return ! ui.sfxOn || ui.sfxOn.checked;
    }
    function playBoxSfx() {
        if (! sfxEnabled()) 
            return;
    ...
    window.addEventListener('ws-game-msg', (ev) => {
        ...
        if (d.type === 'bgm_start' && d.file) { // Sinkronkan tampilan select bgm di non-owner juga (informatif).
            if (ui.bgmSel && ! isOwner) { // Hanya set kalau opsi tersebut sudah dimuat.
                const opt = Array.from(ui.bgmSel.options).find(o => o.value === d.file);
                if (opt) 
                    ui.bgmSel.value = d.file;
    ...
    if (isOwner && ui.bgmSel) {
        ui.bgmSel.addEventListener('change', () => {
            ...
            sendWS({
                type: 'bgm_select',
                room_id: RC.roomId,
                file: ui.bgmSel.value || ''
            });
            if (bgmAudio || bgmCurrentFile) { // Sedang memutar -> ganti track untuk semua orang.
                if (ui.bgmSel.value) 
                    ownerBroadcastBgmStart();
                 else 
                    ownerBroadcastBgmStop();
    ...
    window.addEventListener('ws-game-msg', (ev) => {
        const d = ev.detail;
        if (! d || d.type !== 'bgm_select' || isOwner || ! ui.bgmSel) 
            return;
    ...
    (function loadBgmList() {
        ...
        fetch('assets/bgm/manifest.json', {cache: 'no-store'}).then(r => r.ok ? r.json() : []).then(list => {
            if (!Array.isArray(list)) 
                return;
            


            for (const item of list) { // Item bisa string "nama.mp3" atau {file, label}
                const opt = document.createElement('option');
                if (typeof item === 'string') {
                    opt.value = item;
                    opt.textContent = item.replace(/\.[^.]+$/, '');
                } else {
                    opt.value = item.file;
    ...
    (function setupMobileControls() {
        ...
        if (joyWrap && joyBase && joyKnob) {
            ...
            const endPointer = (e) => {
                ...
                try {
                    joyWrap.releasePointerCapture(activeId);
    ...
    // ---------- Refresh handling saat game sedang berlangsung ----------
    // Server menyimpan flag "room_playing" (helpers/room_playing.php). Saat
    // halaman room dimuat ulang sementara flag aktif:
    // - Owner: anggap reset. Broadcast game_reset supaya non-owner ikut bersih,
    //    flag server di-clear oleh ws/chat-server.php.
    // - Non-owner: kunci layar & scroll ke bawah seperti sedang main, sambil
    //    menunggu snapshot/started berikutnya dari owner.
    (function handleRefreshDuringPlay() {
        if (! RC || ! RC.wasPlaying) 
            return;
        


        if (isOwner) { // Tunggu WS terhubung lalu kirim reset.
            const tryReset = () => {
                const ws = window.__ROOM_WS__;
                if (ws && ws.readyState === 1) {
                    sendWS({type: 'game_reset', room_id: RC.roomId});
                    clearArenaToBlack();
                    setRoomControlsLocked(false);
                    ui.status.textContent = 'Game di-reset karena halaman dimuat ulang.';
                    return;
                }
                setTimeout(tryReset, 200);
            };
            tryReset();
        } else {
            // Non-owner: game tetap berjalan di owner. Lock layar & scroll ke bawah,
            // aktifkan kembali pengiriman input supaya pemain bisa langsung kontrol
            // karakternya begitu snapshot berikutnya tiba.
            setTimeout(() => {
                ui.status.textContent = 'Menyambung kembali ke game...';
                state.running = true;
                try {
                    startInputSender();
                } catch (_) {}setRoomControlsLocked(true);
            }, 150);
        }
    })();
})();
